...
for (let i = 0; i < hoursInDay; i++) {
  const startHour = new Date(currentDate.getFullYear(), currentDate.getMonth(), currentDate.getDate(), i, 0, 0);
...
console.log(hourlyRanges);
const hoursFormatted = hourlyRanges.map(range => ({
  start: range.start,
  end: range.end,
  startNew: new Date(range.start).toLocaleTimeString('ua-UA', { hour: 'numeric', minute: 'numeric', timeZone }),
  endNew: new Date(range.end).toLocaleTimeString('ua-UA', { hour: 'numeric', minute: 'numeric', timeZone }),
}));
